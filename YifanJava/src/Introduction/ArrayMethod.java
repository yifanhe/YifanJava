...
public class ArrayMethod {
	...
	static int[] random = {1,2,3,4,5,6,1,2,3};
	static int[] random1 = {6,5,4};
	 ...
	 public static void main(String[] args) {
	      ...
	      * */
		 searchUnsorted(random,2);
		 searchSorted(random,2);
		 isSorted(random1);
	    }
	     ...
	     * */
	    	for(int index = 0; index<sortedArrayToSearch.length-1;index++){
	    		for(int index1 = 0; index1<sortedArrayToSearch.length-1;index1++){
	    			if(sortedArrayToSearch[index1]<sortedArrayToSearch[index1+1]) //length-1 because or it will be arrayIndexoutofBound
	    			{
	    				int a = index1;
	    				int b = index1+1;
	    				int placeHolder =sortedArrayToSearch[a];
	    				sortedArrayToSearch[a] = sortedArrayToSearch[b];
	    				sortedArrayToSearch[b] = placeHolder;
	    			}
	    		}
	    	}
	    	printArrayContent(sortedArrayToSearch);
	    	
	    	for(int index=0;index<sortedArrayToSearch.length;index++){
		    	 if(sortedArrayToSearch[index] == key){
	    		 System.out.println(index);
	    		 return index;
	    	 }
	     }
	    ...
	    public static int countDifferences(int[] array1, int[] array2){
	         ...
	         * countDifferences({3,2,3,4},{3,2,5,4}) returns 3, since '3','2', and '4' are same value, same location
	         * countDifferences({4,4,4,4},{1,2,3,4}) returns 1, since '4' is only at the same index ONE time
	         * countDifferences({1,2,3},{1,3,2}) returns 1, since '1' is present in both arrays and at the same location. '2' and '3' are both present, but different locations
	         * 
	         * */
	         return 0;
	    }
